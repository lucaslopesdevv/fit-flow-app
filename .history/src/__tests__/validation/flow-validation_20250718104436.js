...
const mockSupabase = {
  auth: {
    ...
    signInWithPassword: () =>
      Promise.resolve({ data: { user: { role: "instructor" } }, error: null }),
  },
  functions: {
...
async function validateCompleteFlow() {
  ...
  try {
    ...
    assert(
      session.data.session?.user.role === "instructor",
    ...
    assert(
      instructorPaths.every((path) => path.includes("(instructor)")),
  ...
  try {
    ...
    assert(
      result.invitation_data.email === invitationData.email,
      "Invitation data contains correct email"
    );
    assert(
      result.invitation_data.instructor_id === "instructor-123",
  ...
  try {
    ...
    assert(
      updateResult.full_name === "Updated Student Name",
  ...
  try {
    ...
    assert(
      instructorSession.user.role === "instructor",
      "Instructor role validation works"
    );

    // Test student access
    const studentSession = { user: { role: "student" } };
    assert(
      studentSession.user.role === "student",
      "Student role validation works"
    );

    // Test role separation
    assert(
      instructorSession.user.role !== studentSession.user.role,
    ...
    assert(
      !validRoles.includes(unknownSession.user.role),
  ...
  try {
    ...
    assert(
      instructorTabs.length === 5,
      "Instructor has correct number of tabs"
    );
    ...
    // Test tab separation
    assert(
      !studentTabs.includes("Students"),
      "Student does not have Students tab"
    );
    assert(
      !instructorTabs.includes("Workouts"),
